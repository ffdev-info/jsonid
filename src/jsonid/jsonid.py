...
@helpers.timeit
async def identify_plaintext_bytestream(path: str) -> Tuple[bool, str, str, str]:
    ...
    for encoding in supported_encodings:
        try:
            ...
            logger.debug("(%s) can't process: '%s', err: %s", encoding, path, err)
        if valid:
            return valid, data, doctype, encoding
    return False, None, None, None
...
async def identify_json(paths: list[str], binary: bool, simple: bool):
    ...
    for idx, path in enumerate(paths):
        if os.path.getsize(path) == 0:
            ...
            continue
        valid, data, doctype, encoding = await identify_plaintext_bytestream(path)
        if not valid:
